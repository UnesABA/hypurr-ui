        ...
        if (data && data.assetPositions) {
          const perpsData = data.assetPositions.map((position) => {
            ...
            return {
              ...
              side: side,
              leverage: leverage,
              value: formatCurrency(parseFloat(position.position.positionValue)),
              amount: `${formatNumber(Math.abs(szi))} ${position.position.coin}-USD`,
              entry: formatCurrency(parseFloat(position.position.entryPx)),
              price: "41.367$", // Hardcoded for now as requested
              pnl: `${parseFloat(position.position.unrealizedPnl) >= 0 ? '+' : ''}${formatCurrency(parseFloat(position.position.unrealizedPnl))}`,
              funding: formatCurrency(parseFloat(position.position.cumFunding.sinceOpen)),
              liquidation: formatCurrency(parseFloat(position.position.liquidationPx)),
    ...
    <>
      ...
      <tbody>
        {loading ? (
          ...
          <tr>
            <td colSpan="10" className="py-12 text-center">
              <span className="text-gray-400 text-sm">No perp positions available</span>
            </td>
          </tr>
        ) : (
